...
package object parsley
{
    ...
    /** This method is responsible for actually executing parsers. Given a `Parsley[A]` and an input
      ...
      */
    def runParser[A](p: Parsley[Char, A], input: String): Result[A] = runParser[Char, A](p, input.toCharArray, None)
    /** This method is responsible for actually executing parsers. Given a `Parsley[A]` and an input
      ...
      * @return Either a success with a value of type `A` or a failure with error message
      */
    def runParser[Tok, A](p: Parsley[Tok, A], input: Array[Tok], pos: Option[Tok => (Int, Int)])(implicit conv: InputPrep[Tok]): Result[A] = new Context(p.threadSafeInstrs, input, pos.asInstanceOf[Option[Any => (Int, Int)]]).runParser()
    /** This method is responsible for actually executing parsers. Given a `Parsley[A]` and an input
      * string, will parse the string with the parser. The result is either a `Success` or a `Failure`.
      * @param p The parser to run
      * @param input The input to run against
      ...
      * @return Either a success with a value of type `A` or a failure with error message
      */
    def runParser[A](p: Parsley[Char, A], input: String, ctx: Context): Result[A] = runParser[Char, A](p, input.toCharArray, ctx, None)
    /** This method is responsible for actually executing parsers. Given a `Parsley[A]` and an input
      * array, will parse the string with the parser. The result is either a `Success` or a `Failure`.
      * @param p The parser to run
      * @param input The input to run against
      ...
      * @return Either a success with a value of type `A` or a failure with error message
      */
    def runParser[Tok, A](p: Parsley[Tok, A], input: Array[Tok], ctx: Context, pos: Option[Tok => (Int, Int)])(implicit conv: InputPrep[Tok]): Result[A] = ctx(p.threadSafeInstrs, input, pos.asInstanceOf[Option[Any => (Int, Int)]]).runParser()
    ...
    def runParserFastUnsafe[A](p: Parsley[Char, A], input: String)(implicit ctx: Context = internalCtx): Result[A] = runParserFastUnsafe[Char, A](p, input.toCharArray, ctx, None)
    private [parsley] def runParserFastUnsafe[Tok, A](p: Parsley[Tok, A], input: Array[Tok], ctx: Context, pos: Option[Tok => (Int, Int)])(implicit conv: InputPrep[Tok]): Result[A] = ctx(p.instrs, input, pos.asInstanceOf[Option[Any => (Int, Int)]]).runParser()
    ...
    /**
      ...
      */
    def runParserThreadSafe[A](p: Parsley[Char, A], input: String, ctx: Context = giveContext): Result[A] = ctx(p.threadSafeInstrs, input.toCharArray, None).runParser()
    ...
    /**
      ...
      */
    def giveContext: Context = new Context(null, Array.empty, None)
